...
// Uses useNetworkWriteSafety, which checks /api/write-safety/status before enabling writes.
...
import { useNetworkWriteSafety, WriteDisabledNotice } from '../../lib/writeSafety';
...
export default function UpsPage({ onNavigate }) {
  ...
  const { writeDisabled } = useNetworkWriteSafety();
    ...
    },
    {
      ...
      render: (install) => (
        <div className={styles.rowActions}>
          <button type="button" className="dangerButton compactButton" onClick={(event) => handleRollbackFromRow(event, install)} disabled={writeDisabled}>
            Remove
          </button>
        </div>
      )
  ...
  function guardWriteAction() {
    ...
    notify('warning', 'Write operations are disabled');
    return true;
  }
  ...
  return (
    <>
      ...
      {writeDisabled && <WriteDisabledNotice />}
          ...
          actions={
            <div className={styles.sectionActions}>
              ...
              {selectedPendingIds.size > 0 && (
                <button type="button" className="primaryButton" onClick={openScheduleModal} disabled={writeDisabled}>
        ...
        <SectionCard
          ...
          actions={
            <div className={styles.sectionActions}>
              ...
              {visibleWarehouseSelectableIds.length > 0 && (
                <button type="button" className="secondaryButton" onClick={handleToggleVisibleScheduledSelection} disabled={writeDisabled}>
                  {allVisibleWarehouseSelected ? 'Clear All' : 'Select All'}
                </button>
              )}
              {allSelectedWarehouseReady && (
                <button type="button" className="primaryButton" onClick={openWarehouseModal} disabled={writeDisabled}>
                  Copy Warehouse Table
                </button>
              )}
              {allSelectedConfirmIp && (
                <button type="button" className="primaryButton" onClick={openBatchIpConfirmModal} disabled={writeDisabled}>
                  Copy IP Response
                </button>
              )}
      ...
      {scheduleModalOpen && (
        <Modal title="NOC Schedule" onClose={closeScheduleModal}>
          ...
          <div className={styles.actions}>
            <button type="button" className="primaryButton" onClick={handleMoveToInProgress} disabled={writeDisabled}>
              Move to In Progress
            </button>
      ...
      {warehouseModalOpen && (
        <Modal title="Warehouse Email Preview" onClose={closeWarehouseModal}>
          ...
          <div className={styles.actions}>
            <button type="button" className="primaryButton" onClick={handleCopyWarehouseTable} disabled={writeDisabled}>
              Copy Warehouse Table
            </button>
          ...
          <SpotlightPanel as="form" className={styles.serviceForm} mode="interactive" onSubmit={handleSaveFulfillment}>
            <div className={styles.serviceGrid}>
              <label>
                ...
                <input value={fulfillmentForm.new_asset_tag} onChange={(event) => updateFulfillmentForm('new_asset_tag', event.target.value)} maxLength={100} disabled={writeDisabled} />
              </label>
              <label>
                UPS SN
                <input value={fulfillmentForm.new_serial_number} onChange={(event) => updateFulfillmentForm('new_serial_number', event.target.value)} maxLength={100} disabled={writeDisabled} />
              </label>
              <label>
                SNMPWEBCARD SN
                <input value={fulfillmentForm.new_webcard_serial} onChange={(event) => updateFulfillmentForm('new_webcard_serial', event.target.value)} maxLength={100} disabled={writeDisabled} />
              </label>
              <label>
                Replacement MAC
                <input value={fulfillmentForm.new_mac_address} onChange={(event) => updateFulfillmentForm('new_mac_address', event.target.value)} maxLength={32} disabled={writeDisabled} />
              </label>
              <label>
                BP SN
                <input value={fulfillmentForm.new_battery_pack_serial} onChange={(event) => updateFulfillmentForm('new_battery_pack_serial', event.target.value)} maxLength={100} disabled={writeDisabled} />
              </label>
              <label>
                BP Asset Tag #
                <input value={fulfillmentForm.new_battery_pack_asset_tag} onChange={(event) => updateFulfillmentForm('new_battery_pack_asset_tag', event.target.value)} maxLength={100} disabled={writeDisabled} />
              </label>
            </div>
            <div className={styles.actions}>
              <button type="submit" className="primaryButton" disabled={writeDisabled}>Save Fulfillment Details</button>
        ...
        <Modal title="Confirm UPS IP" onClose={closeIpConfirmModal}>
          ...
          <SpotlightPanel as="section" className={styles.serviceForm} mode="interactive">
            <div className={styles.serviceGrid}>
              <label>
                SNMP IP
                <input value={ipConfirmForm.snmp_ip} onChange={(event) => updateIpConfirmForm('snmp_ip', event.target.value)} maxLength={100} disabled={writeDisabled} />
              </label>
              <label>
                Device Name / Hostname
                <input value={ipConfirmForm.hostname} onChange={(event) => updateIpConfirmForm('hostname', event.target.value)} maxLength={100} disabled={writeDisabled} />
              </label>
              <label>
                SNMP MAC
                <input value={ipConfirmForm.new_mac_address} onChange={(event) => updateIpConfirmForm('new_mac_address', event.target.value)} maxLength={32} disabled={writeDisabled} />
              </label>
              <label>
                SNMPWEBCARD SN
                <input value={ipConfirmForm.new_webcard_serial} onChange={(event) => updateIpConfirmForm('new_webcard_serial', event.target.value)} maxLength={100} disabled={writeDisabled} />
              </label>
              <label>
                Replacement Asset Tag #
                <input value={ipConfirmForm.new_asset_tag} onChange={(event) => updateIpConfirmForm('new_asset_tag', event.target.value)} maxLength={100} disabled={writeDisabled} />
              </label>
              <label>
                UPS SN
                <input value={ipConfirmForm.new_serial_number} onChange={(event) => updateIpConfirmForm('new_serial_number', event.target.value)} maxLength={100} disabled={writeDisabled} />
              </label>
              <label>
                UPS PO
                <input value={ipConfirmForm.ups_po} onChange={(event) => updateIpConfirmForm('ups_po', event.target.value)} maxLength={100} disabled={writeDisabled} />
              </label>
              <label>
                BP SN
                <input value={ipConfirmForm.new_battery_pack_serial} onChange={(event) => updateIpConfirmForm('new_battery_pack_serial', event.target.value)} maxLength={100} disabled={writeDisabled} />
              </label>
              <label>
                BP Asset Tag #
                <input value={ipConfirmForm.new_battery_pack_asset_tag} onChange={(event) => updateIpConfirmForm('new_battery_pack_asset_tag', event.target.value)} maxLength={100} disabled={writeDisabled} />
              </label>
              <label>
                BP PO
                <input value={ipConfirmForm.bp_po} onChange={(event) => updateIpConfirmForm('bp_po', event.target.value)} maxLength={100} disabled={writeDisabled} />
              </label>
            </div>
            <div className={styles.actions}>
              <button type="button" className="primaryButton" onClick={handleCopyIpResponseTable} disabled={completingIpConfirm || writeDisabled}>
      ...
      {batchIpConfirmModalOpen && (
        <Modal title="Confirm UPS IP Response" onClose={closeBatchIpConfirmModal}>
            ...
            {batchIpConfirmRows.map((row) => (
              <SpotlightPanel key={row.ups_installation_id} className={styles.serviceForm} mode="interactive">
                ...
                <div className={styles.serviceGrid}>
                  <label>
                    Device Name / Hostname
                    <input value={row.form.hostname} onChange={(event) => updateBatchIpConfirmRow(row.ups_installation_id, 'hostname', event.target.value)} maxLength={100} disabled={writeDisabled} />
                  </label>
                  <label>
                    SNMP IP
                    <input value={row.form.snmp_ip} onChange={(event) => updateBatchIpConfirmRow(row.ups_installation_id, 'snmp_ip', event.target.value)} maxLength={100} disabled={writeDisabled} />
                  </label>
                  <label>
                    SNMP MAC
                    <input value={row.form.new_mac_address} onChange={(event) => updateBatchIpConfirmRow(row.ups_installation_id, 'new_mac_address', event.target.value)} maxLength={32} disabled={writeDisabled} />
                  </label>
                  <label>
                    Replacement Asset Tag #
                    <input value={row.form.new_asset_tag} onChange={(event) => updateBatchIpConfirmRow(row.ups_installation_id, 'new_asset_tag', event.target.value)} maxLength={100} disabled={writeDisabled} />
                  </label>
                  <label>
                    UPS SN
                    <input value={row.form.new_serial_number} onChange={(event) => updateBatchIpConfirmRow(row.ups_installation_id, 'new_serial_number', event.target.value)} maxLength={100} disabled={writeDisabled} />
                  </label>
                  <label>
                    UPS PO
                    <input value={row.form.ups_po} onChange={(event) => updateBatchIpConfirmRow(row.ups_installation_id, 'ups_po', event.target.value)} maxLength={100} disabled={writeDisabled} />
                  </label>
                  <label>
                    BP SN
                    <input value={row.form.new_battery_pack_serial} onChange={(event) => updateBatchIpConfirmRow(row.ups_installation_id, 'new_battery_pack_serial', event.target.value)} maxLength={100} disabled={writeDisabled} />
                  </label>
                  <label>
                    BP Asset Tag #
                    <input value={row.form.new_battery_pack_asset_tag} onChange={(event) => updateBatchIpConfirmRow(row.ups_installation_id, 'new_battery_pack_asset_tag', event.target.value)} maxLength={100} disabled={writeDisabled} />
                  </label>
                  <label>
                    BP PO
                    <input value={row.form.bp_po} onChange={(event) => updateBatchIpConfirmRow(row.ups_installation_id, 'bp_po', event.target.value)} maxLength={100} disabled={writeDisabled} />
                  </label>
                </div>
              </SpotlightPanel>
            ))}
          ...
          <div className={styles.actions}>
            <button type="button" className="primaryButton" onClick={handleCopyBatchIpResponseTable} disabled={batchIpConfirmCopying || writeDisabled}>
      ...
      {completedSummaryInstall && (
        <Modal title="UPS Details" onClose={closeCompletedSummaryModal}>
          ...
          <div className={styles.actions}>
            {completedSummaryEditing ? (
              <>
                <button type="button" className="primaryButton" onClick={handleSaveCompletedSummary} disabled={writeDisabled}>Save</button>
              ...
              <>
                {completedSummaryInstall.status === 'fulfilled' && (
                  <button type="button" className="correctionButton" onClick={openReturnToServicingConfirm} disabled={writeDisabled}>Return to Servicing</button>
                )}
                <button type="button" className="secondaryButton" onClick={() => setCompletedSummaryEditing(true)} disabled={writeDisabled}>Edit</button>
              </>
            )}
      ...
      {returnToServicingConfirmOpen && (
        <Modal title="Return to Servicing" onClose={closeReturnToServicingConfirm}>
          ...
          <div className={styles.actions}>
            ...
            <button type="button" className="primaryButton" onClick={handleReturnToServicing} disabled={returningToServicing || writeDisabled}>
