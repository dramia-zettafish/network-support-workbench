...
const TABLES = {
  tickets: 'network_workbench.tickets',
  rmas: 'network_workbench.rmas',
  deviceResponses: 'network_workbench.device_responses',
  upsInstallations: 'network_workbench.ups_installations'
};
...
export async function createTicket(payload) {
  ...
  return transaction(async (client) => {
    ...
    if (createdTicket.device_type === 'ups') {
      await client.query(
        `
          INSERT INTO ${TABLES.upsInstallations} (
            ticket_number,
            external_ticket_number,
            school_name,
            tea_code,
...
export async function deleteTicket(ticketNumber) {
  return transaction(async (client) => {
    await client.query(`DELETE FROM ${TABLES.upsInstallations} WHERE ticket_number = $1`, [ticketNumber]);
    const result = await client.query(`DELETE FROM ${TABLES.tickets} WHERE ticket_number = $1 RETURNING ticket_number`, [ticketNumber]);
...
export async function getTicketResponse(ticketNumber) {
  const result = await query(
    `SELECT ${responseColumns} FROM ${TABLES.deviceResponses} WHERE ticket_id = $1`,
    [ticketNumber]
  );
...
export async function createTicketResponse(ticketNumber, payload) {
  ...
  return transaction(async (client) => {
    const ticket = await client.query(`SELECT ticket_number FROM ${TABLES.tickets} WHERE ticket_number = $1`, [ticketNumber]);
    if (!ticket.rows[0]) {
      throw Object.assign(new Error('Ticket not found'), { status: 404 });
    }

    const existing = await client.query(`SELECT id FROM ${TABLES.deviceResponses} WHERE ticket_id = $1`, [ticketNumber]);
    if (existing.rows[0]) {
    ...
    const result = await client.query(
      `
        INSERT INTO ${TABLES.deviceResponses} (ticket_id, ${fields.join(', ')})
...
async function getTicketOrThrow(ticketNumber) {
  const result = await query(`SELECT ${ticketColumns} FROM ${TABLES.tickets} WHERE ticket_number = $1`, [ticketNumber]);
  if (!result.rows[0]) {
    throw Object.assign(new Error('Ticket not found'), { status: 404 });
  }
  return result.rows[0];
