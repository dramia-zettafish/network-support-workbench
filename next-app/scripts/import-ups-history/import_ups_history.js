...
const defaultCsvPath = path.join(__dirname, 'ups_clean_import_ready_with_2026_additions_NO_REVIEW_NEEDED.csv');

const insertSql = `
  INSERT INTO network_workbench.ups_installations (
    ticket_number,
...
const updateSql = `
  UPDATE network_workbench.ups_installations
  SET
...
async function main() {
  ...
  if (!connectionString) {
    throw new Error('NETWORK_DATABASE_URL must be set. Historical UPS import never falls back to the EUS database.');
  }
...
function printUsage() {
  ...
  console.log('       Set NETWORK_DATABASE_URL to target the merged Network Workbench database.');
}
    ...
    result = await pool.query(
      `
        SELECT
          ...
          bp_po,
          proposed_install_date
        FROM network_workbench.ups_installations
        WHERE
